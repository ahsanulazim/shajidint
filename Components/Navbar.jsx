...
export default function Navbar({ glass, user }) {
  ...
  return (
    <header
      className={`drawer transition-all duration-300 fixed z-10 ${
        glass ? "bg-black/80 backdrop-blur-xs" : "bg-transparent"
      }`}
      data-theme="dark"
    >
      ...
      <div className="drawer-content flex flex-col">
        ...
        <div className="navbar lg:px-10 w-full max-w-[1440px] mx-auto md:min-h-20">
          ...
          <div className="navbar-center hidden lg:flex">
            <ul className="menu menu-horizontal px-1 font-medium">{nav}</ul>
          </div>
          <div className="navbar-end">
            {user ? (
              <Link href="/dashboard" className="">
                <button className="btn bg-white text-black rounded-md ms-2">
                  Dashboard
                </button>
              </Link>
            ) : (
